...
#define LANDSCAPE_RESOLUTION 32
...
unsigned int  difficulty = 15;
         int  theDelay = 20; 
         int  landscape_top[32];
         int  landscape_bottom[32];
         int  player_track[32];
         int  value_player = 0;
         int  value_top = 0;
         int  value_bottom = 0;
...
void setup()
{
  ...
  for(int x=0;x<LANDSCAPE_RESOLUTION;x++){
    player_track[x] = 32;
    landscape_top[x] = 22;
    landscape_bottom[x] = 42;
  }
...
{  
  ...
  // Move the landscape left
  for(int x=1;x<LANDSCAPE_RESOLUTION;x++){
    player_track[x-1] = player_track[x];
    landscape_top[x-1] = landscape_top[x];
    landscape_bottom[x-1] = landscape_bottom[x];
  }
  
  
  value_player = map(analogRead(5),0,1024,0,63);
  player_track[8] = value_player;
    
  value_top = landscape_top[LANDSCAPE_RESOLUTION-2] + (random(0,5) - 2);
  value_bottom = landscape_bottom[LANDSCAPE_RESOLUTION-2] + (random(0,5) - 2);
  if(value_top < 0) value_top = 0;
  if(value_top > 63) value_top = 63;
  if(value_bottom < 0) value_bottom = 0;
  if(value_bottom > 63) value_bottom = 63;
  

  if(value_bottom - value_top < difficulty){
    landscape_top[LANDSCAPE_RESOLUTION-1] = landscape_top[LANDSCAPE_RESOLUTION-2];
    landscape_bottom[LANDSCAPE_RESOLUTION-1] = landscape_bottom[LANDSCAPE_RESOLUTION-2];
  }
  else
  {
    landscape_top[LANDSCAPE_RESOLUTION-1] = value_top;
    landscape_bottom[LANDSCAPE_RESOLUTION-1] = value_bottom;
  }

  Serial.print("Top: ");
  Serial.print(value_top);
  Serial.print(" - ");
  Serial.print(landscape_top[LANDSCAPE_RESOLUTION-1]);
  Serial.print(" Bottom: ");
  Serial.print(value_bottom);
  Serial.print(" - ");
  Serial.println(landscape_bottom[LANDSCAPE_RESOLUTION-1]);
  
  GLCD.DrawLine(29,value_player-1,30,value_player+1);
  GLCD.DrawLine(30,value_player-1,29,value_player+1);
  
  // Draw landscape
  for(int x=1;x<LANDSCAPE_RESOLUTION;x++){

    if(x<6)
      GLCD.DrawLine(x*4,player_track[x-1], (x+1)*4, player_track[x]);

    GLCD.DrawLine(x*4,landscape_top[x-1], (x+1)*4, landscape_top[x]);
    GLCD.DrawLine(x*4,landscape_bottom[x-1], (x+1)*4, landscape_bottom[x]);

    //GLCD.DrawLine(x*4,0, (x+1)*4, landscape_top[x]);
    //GLCD.DrawLine(x*4,63, (x+1)*4, landscape_bottom[x]);
    GLCD.DrawLine((x+1)*4,0, x*4, landscape_top[x-1]);
    GLCD.DrawLine((x+1)*4,63, x*4, landscape_bottom[x-1]);
